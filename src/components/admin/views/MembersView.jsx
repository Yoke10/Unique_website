...
const MembersView = () => {
    ...
    const loadUsers = async () => {
        try { setUsers(await mockDataService.getUsers()); }
    ...
    const handleEditSave = async () => {
        try {
            const { username, role, ...profileData } = editForm;
            await mockDataService.updateUser(selectedUser.id, { username, role, profile: profileData });
            ...
            <AdminModal isOpen={!!selectedUser} onClose={() => setSelectedUser(null)} title={isEditing ? "Edit User" : "User Details"} size="large">
                ...
                <div className="detail-grid-3">
                    ...
                    <div style={{ gridColumn: '1/-1' }}>
                        <AdminInput label="Address" value={editForm.addressLine1 || ''} disabled={!isEditing} onChange={e => setEditForm({ ...editForm, addressLine1: e.target.value })} />
                    </div>
                </div>
            </AdminModal>
        </div>
